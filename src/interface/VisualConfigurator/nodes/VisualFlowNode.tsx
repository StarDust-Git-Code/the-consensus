...
export const VisualFlowNode = ({ data, selected, type }: any) => {
  ...
  return (
    <div
      className={`
        relative px-3 py-2.5 shadow-sm rounded-2xl border-2 pointer-events-auto transition-all duration-300 w-fit min-w-[280px]
      ...
      style={{
        ...
        borderColor: selected ? (isUser ? USER_COLOR : data.color) : (isUser ? USER_COLOR : data.color || 'var(--border-subtle)'),
        boxShadow: selected ? `0 10px 15px -3px rgba(0,0,0,0.5), 0 4px 6px -2px rgba(0,0,0,0.3), 0 0 0 4px ${isUser ? USER_COLOR : data.color}30` : undefined
      }}
    >
      {/* Handles */}
      ...
      <div className="flex items-center gap-3">
        <div className="shrink-0 p-0.5 rounded-xl border" style={{ backgroundColor: 'var(--bg-elevated)', borderColor: 'var(--border-subtle)' }}>
          <Avatar
        ...
        <div className="flex flex-col min-w-0 flex-1 gap-1">
          <div
            ...
            style={{ color: isUser ? USER_COLOR : 'var(--text-primary)' }}
          >
            {data.label}
          </div>
