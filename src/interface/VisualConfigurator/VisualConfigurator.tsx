...
const InternalHeader = ({ onClose, system }: { onClose: () => void, system: any }) => (
  <div className="h-14 border-b flex items-center justify-between px-6 z-50 shrink-0" style={{ backgroundColor: 'var(--bg-elevated)', borderColor: 'var(--border-subtle)' }}>
    <div className="flex items-center gap-2">
      <Settings size={18} strokeWidth={2} style={{ color: 'var(--text-primary)' }} />
      <h2 className="text-xs font-black uppercase tracking-[0.2em] ml-2" style={{ color: 'var(--text-primary)' }}>Manage Teams</h2>
    ...
    <div className="flex items-center gap-3">
      <button
        onClick={onClose}
        className="p-2 hover:bg-zinc-800 rounded-xl transition-colors group border border-transparent hover:border-zinc-700"
      >
        <X className="w-5 h-5 text-zinc-400 group-hover:text-zinc-200" />
      </button>
    </div>
  </div>
);
...
const AgentPlaceholder = () => (
  <div className="w-80 flex flex-col items-center justify-center p-8 text-center" style={{ color: 'var(--text-muted)' }}>
    <User size={32} strokeWidth={1.5} className="mb-4 opacity-20" />
    <p className="text-[10px] uppercase font-bold tracking-widest" style={{ color: 'var(--text-secondary)' }}>Select an agent</p>
...
const VisualConfiguratorContent: React.FC = () => {
  ...
  return (
    <div className="w-full h-full relative flex flex-col overflow-hidden" style={{ backgroundColor: 'var(--bg-surface)' }}>
      <InternalHeader onClose={() => handleClose(false)} system={system} />

      <div className="flex-1 min-h-0 relative flex overflow-hidden">
        {/* Left Panel: Agent Config */}
        <div className="relative shrink-0 flex border-r" style={{ backgroundColor: 'var(--bg-elevated)', borderColor: 'var(--border-subtle)' }}>
            ...
            {configMode === 'edit' && selectedAgentId && selectedAgentId !== 'user' && (
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 z-10 animate-in fade-in slide-in-from-bottom-4 duration-300">
                <button
                  ...
                  className={`flex items-center gap-2 px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.1em] border transition-all ${characters.length >= MAX_AGENTS + 1
                    ? 'opacity-40 cursor-not-allowed grayscale'
                    : 'hover:scale-105 active:scale-95 hover:bg-zinc-800'
                    }`}
