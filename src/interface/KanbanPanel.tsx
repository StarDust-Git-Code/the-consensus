...
function TaskCard({ task }: { task: Task; key?: string }) {
  ...
  return (
    <div key={task.id} className="rounded-lg border shadow-sm p-3 space-y-2 group relative" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-subtle)' }}>
      <div
        className="flex items-start justify-between gap-1 cursor-pointer"
        onClick={() => setIsExpanded(!isExpanded)}
      >
        <h3 className="text-xs leading-snug font-bold flex-1" style={{ color: 'var(--text-primary)' }}>
          ...
          {task.status !== 'done' && (
            <>
              <button
                ...
                className="p-1 text-zinc-600 hover:text-red-400 hover:bg-red-900/20 rounded transition-all"
                title="Remove task"
              >
                <Trash2 size={12} />
              </button>
      ...
      {isExpanded && (
        <p className="text-[11px] leading-relaxed p-2 rounded border animate-in fade-in slide-in-from-top-1 duration-200" style={{ color: 'var(--text-secondary)', backgroundColor: 'var(--bg-elevated)', borderColor: 'var(--border-subtle)' }}>
          {task.description}
        </p>
      )}
        ...
        <div className="flex items-center gap-2">
          ...
          {(task.status === 'done' || task.draftOutput || (task.revisions && task.revisions.length > 0)) && (
            <button
              ...
              className="p-1 px-2 text-zinc-500 hover:text-emerald-400 hover:bg-emerald-900/20 rounded-lg transition-all flex items-center gap-1.5 group/audit"
...
export function KanbanPanel({ height = 320 }: KanbanPanelProps) {
  ...
  return (
    <div
      className="w-full border-t flex flex-col pointer-events-auto shrink-0 relative"
      style={{ height, backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-subtle)' }}
    >
      {/* Columns Scroll Area */}
              ...
              <div key={status} className="w-52 flex flex-col gap-3">
                <div className="flex items-center justify-between shrink-0 select-none">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-1.5 py-0.5 text-zinc-500 text-[9px] font-bold rounded-md min-w-4.5 text-center" style={{ backgroundColor: 'var(--bg-card)' }}>
                      {colTasks.length}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex-1 flex flex-col gap-2 overflow-y-auto pr-1">
                  ...
                  {colTasks.length === 0 && (
                    <div className="border border-dashed rounded-lg p-4 flex items-center justify-center select-none" style={{ borderColor: 'var(--border-subtle)' }}>
