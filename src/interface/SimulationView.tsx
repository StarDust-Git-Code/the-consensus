...
const SimulationView: React.FC<SimulationViewProps> = ({ canvasRef, isFullscreen, setIsFullscreen }) => {
  ...
  return (
    <div className="flex flex-col flex-1 min-w-0 min-h-0 relative">
      ...
      <div className="h-14 border-b flex items-center justify-between px-5 shrink-0" style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-subtle)' }}>
        <div className="flex-1 flex items-center gap-4">
          <button
            onClick={() => setIsFlowModalOpen(true)}
            className="flex items-center gap-4 hover:bg-zinc-800 px-2.5 py-1.5 rounded-2xl transition-all active:scale-95 group cursor-pointer"
            title="View Team Flow"
          >
            <TeamBadge system={activeSet} />
              <div className="w-8 h-8 rounded-full border flex items-center justify-center text-zinc-500 group-hover:text-zinc-300 group-hover:border-zinc-600 transition-colors" style={{ borderColor: 'var(--border-subtle)' }}>
              <Eye size={14} />
            </div>
          </button>
