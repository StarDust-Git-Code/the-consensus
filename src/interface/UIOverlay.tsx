...
const AlertBubble: React.FC<AlertBubbleProps> = ({ icon, position, visible, color = '#facc15', onClick }) => {
  ...
  return (
    ...
    >
      <div
        className={`bg-darkConsensus/90 backdrop-blur-md p-1.5 rounded-full border border-white/10 shadow-xl flex items-center justify-center hover:scale-110 active:scale-95 transition-transform ${onClick ? 'hover:border-white/30' : ''}`}
        style={{ color }}
      >
        {icon}
      </div>
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-10 overflow-hidden select-none">
      ...
      {(() => {
        ...
        if (selectedAgent && selectedPosition) {
          ...
          return (
            ...
            >
              <div className="backdrop-blur-md px-3 py-1.5 rounded-full border shadow-xl flex items-center gap-2 whitespace-nowrap animate-in fade-in zoom-in-95 duration-200" style={{ backgroundColor: 'rgba(24, 24, 27, 0.9)', borderColor: 'var(--border-subtle)' }}>
                <div
                ...
                <div className="flex items-center gap-1.5">
                  {selectedAgent.index === system.user.index ? (
                    <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: 'var(--text-primary)' }}>{selectedAgent.name} (You)</span>
                    ...
                    <>
                      ...
                      {label.text && (
                        <>
                          <span className="text-[10px] font-medium uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>·</span>
              ...
              <div className="backdrop-blur-md px-3 py-1.5 rounded-full border shadow-xl flex items-center gap-2 whitespace-nowrap animate-in fade-in zoom-in-95 duration-200" style={{ backgroundColor: 'rgba(24, 24, 27, 0.9)', borderColor: 'var(--border-subtle)' }}>
                ...
                <div className="flex items-center gap-1.5">
                  {hoveredAgent.index === system.user.index ? (
                    <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: 'var(--text-primary)' }}>{hoveredAgent.name} (You)</span>
                  ) : isLeadAgentProjectReady ? (
                    <span className={`text-[10px] font-black uppercase tracking-widest ${label.className}`}>
                      {label.text}
                    </span>
      ...
      {hoveredPoiLabel && hoverPosition && (
        ...
        >
          <div className="backdrop-blur-md px-3 py-1.5 rounded-full border shadow-xl flex items-center gap-2 whitespace-nowrap animate-in fade-in zoom-in-95 duration-200" style={{ backgroundColor: 'rgba(24, 24, 27, 0.9)', borderColor: 'var(--border-subtle)' }}>
            <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: 'var(--text-primary)' }}>{hoveredPoiLabel}</span>
          </div>
        </div>
      )}
