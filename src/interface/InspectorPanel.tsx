...
const InspectorPanel: React.FC<InspectorPanelProps> = ({ isFloating }) => {
  ...
  return (
    <div className={`${isFloating ? 'w-full h-full max-h-[85vh] self-end rounded-2xl shadow-2xl border border-zinc-800' : 'w-80 h-full border-l'} flex flex-col pointer-events-auto shrink-0 relative z-30 overflow-hidden transition-all duration-300` } style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-subtle)' }}>
      {!agent ? (
        !isFloating && <ProjectView />
      ) : (
        <>
          {/* Header Section */}
          <div className={`px-4 py-3 border-b ${isFloating ? '' : ''}`} style={{ borderColor: 'var(--border-subtle)', backgroundColor: 'var(--bg-surface)' }}>
            <div className="flex flex-col gap-4">
              {/* Agent Title Row */}
              <div className="flex items-center gap-4">
                <div className="shrink-0 rounded-2xl p-0.5 border" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-subtle)' }}>
                  <Avatar
                ...
                <div className="flex flex-col min-w-0">
                  <h2 className="text-xl font-black leading-tight truncate" style={{ color: 'var(--text-primary)' }}>
                    {agent.name}
                  </h2>
              ...
              {needsInput && isChatting && (
                ...
                >
                  ...
                  <p className="text-[12px] font-bold leading-tight mt-1.5" style={{ color: 'var(--text-primary)' }}>
                    {isLeadAgentIdle
              ...
              {needsInput && !isChatting ? (
                <div className="flex flex-col gap-3 p-4 border rounded-xl animate-in fade-in slide-in-from-top-1 shadow-sm" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-subtle)' }}>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {isLeadAgentIdle && (system.outputType === 'image' || system.outputType === 'video') && (
                      <div className="mt-1 pt-3 border-t" style={{ borderColor: 'var(--border-subtle)' }}>
                        <ReferenceImages />
                      </div>
                    )}
                    ...
                    <button
                      ...
                      className="flex items-center justify-center gap-2 bg-zinc-700 hover:bg-zinc-600 active:scale-95 disabled:opacity-50 text-zinc-200 px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm mt-1"
                    >
                      {isLeadAgentIdle ? (
                        <>
                ...
                <div className="w-full">
                  {agent.index === system.user.index ? (
                    ...
                    <div className="border border-yellow-800/30 rounded-xl p-4 flex flex-col gap-3 shadow-sm" style={{ backgroundColor: 'rgba(250, 204, 21, 0.08)' }}>
                      <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`w-full h-10 px-4 rounded-xl flex items-center justify-center gap-2 transition-all active:scale-95 text-[10px] font-black uppercase tracking-widest ${canChat
                          ? 'bg-zinc-700 text-zinc-200 border-none shadow-md hover:bg-zinc-600'
                          : 'text-zinc-600 border cursor-not-allowed'
                        }`}
                      style={!canChat ? { backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-subtle)' } : undefined}
                    >
                      {canChat ? (
                        <>
          ...
          <div className={`flex-1 overflow-y-auto relative min-h-0`} style={{ backgroundColor: 'var(--bg-root)' }}>
            {isChatting ? (
              <div className="flex flex-col h-full" style={{ backgroundColor: 'var(--bg-surface)' }}>
                ...
                <div className="p-3 border-t shrink-0" style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-subtle)' }}>
                  <button
                    onClick={handleEndChat}
                    className="w-full h-10 px-4 bg-zinc-700 hover:bg-zinc-600 text-zinc-200 rounded-xl flex items-center justify-center gap-2 transition-all active:scale-95 text-[10px] font-black uppercase tracking-widest shadow-md"
                  >
