...
/** Thrown when a model returns 429 / RESOURCE_EXHAUSTED */
export class RateLimitError extends Error {
  public readonly model: string;
  constructor(model: string, message: string) {
    super(`Rate limited on ${model}: ${message}`);
    this.model = model;
...
export class GeminiProvider implements LLMProvider {
  ...
  async generateCompletion(
    ...
    // Gemma models don't support function calling — strip tools
    const isGemma = modelName.toLowerCase().startsWith('gemma');
    const effectiveTools = isGemma ? undefined : tools;

    const systemTools: Tool[] | undefined = effectiveTools ? [{
      functionDeclarations: effectiveTools.map(t => ({
    ...
    try {
      ...
      // Detect rate limit errors and wrap them
      if (error?.status === 429
          || error?.message?.includes('429')
          || error?.message?.includes('RESOURCE_EXHAUSTED')) {
        throw new RateLimitError(modelName, error.message || 'Rate limited');
      }
      throw error;
    }
    ...
    let thinkingStr: string | null = null;
    let toolCalls: LLMToolCall[] = [];

    for (const part of parts) {
      if (part.text) {
        // Gemma 4 models include thinking/reasoning parts with thought=true
        // Filter these out — only include the actual response text
        if ((part as any).thought === true) {
          thinkingStr = (thinkingStr || '') + part.text;
        } else {
