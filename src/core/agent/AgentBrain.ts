...
import { GeminiProvider, RateLimitError } from '../llm/providers/GeminiProvider';
import { LiveProvider } from '../llm/providers/LiveProvider';
...
import { DEFAULT_MODELS, TaskCategory, LIVE_MODELS, MODEL_ROUTING } from '../llm/constants';
...
export class AgentBrain {
  ...
  public async think(prompt: string, options: ThinkOptions = {}): Promise<{ text: string, toolCalls: any[] }> {
    ...
    try {
      ...
      // Sync API keys to ModelRouter
      const allKeys = llmConfig.apiKeys?.length
        ? llmConfig.apiKeys
        : llmConfig.apiKey ? [llmConfig.apiKey] : [];
      if (allKeys.length === 0) throw new Error('Gemini API key is required');
      ModelRouter.setApiKeys(allKeys);

      // Smart model routing: infer category and resolve best model
      const category: TaskCategory = options.taskCategory
        || (options.isChat ? 'chat' : 'execute');
      const agentOverride = this.host.data.model && this.host.data.model !== DEFAULT_MODELS.text
        ? this.host.data.model : undefined;
      const model = ModelRouter.resolveModel(category, agentOverride);

      // Get the best API key for this model (round-robin across keys)
      const { key: activeKey, keyIndex: activeKeyIdx } = ModelRouter.getNextApiKey(model);
      const provider = new GeminiProvider(activeKey);
      ...
      if (category === 'chat') {
        // ── Live API path (unlimited RPD) ──
        // Try Live API first, fall back to standard if it fails or returns empty
        let usedLive = false;
        try {
          const liveProvider = new LiveProvider(activeKey);
          response = await liveProvider.generateChatResponse(
            messages,
            undefined,
            systemPrompt,
            LIVE_MODELS.primary
          );
          actualModel = LIVE_MODELS.primary;

          // Check if Live API actually returned content
          if (response.content && response.content.trim().length > 0) {
            usedLive = true;
          } else {
            console.warn('[AgentBrain] Live API returned empty response, falling back to standard API');
          }
        } catch (liveError) {
          console.warn('[AgentBrain] Live API failed, falling back to standard:', liveError);
        }

        // Fallback to standard generateContent if Live didn't work
        if (!usedLive) {
          // Chat agents may need tools (e.g. set_user_brief).
          // Gemma can't use tools → use a tool-capable model from orchestrate chain.
          const isGemma = model.toLowerCase().startsWith('gemma');
          if (isGemma && toolDefs && toolDefs.length > 0) {
            actualModel = ModelRouter.resolveModel('orchestrate');
            const { key: toolKey, keyIndex: toolKeyIdx } = ModelRouter.getNextApiKey(actualModel);
            const toolProvider = new GeminiProvider(toolKey);
            response = await toolProvider.generateCompletion(messages, toolDefs, systemPrompt, actualModel);
            ModelRouter.recordRequest(actualModel, toolKeyIdx);
          } else {
            actualModel = model;
            response = await provider.generateCompletion(messages, toolDefs, systemPrompt, model);
            ModelRouter.recordRequest(model, activeKeyIdx);
          }
        }
      } else {
        // ── Standard API path with 429 fallback ──
        const fallbackChain = MODEL_ROUTING[category].filter(m => m !== model);
        try {
          response = await provider.generateCompletion(messages, toolDefs, systemPrompt, actualModel);
          ModelRouter.recordRequest(actualModel, activeKeyIdx);
        } catch (error) {
          if (error instanceof RateLimitError) {
            ModelRouter.markExhausted(error.model, activeKeyIdx);
            core.addLogEntry({
              agentIndex: this.host.data.index,
              action: `⚠️ Rate limited on ${error.model} (key #${activeKeyIdx + 1}), trying fallback...`
            });
            let retried = false;
            for (const fallback of fallbackChain) {
              if (ModelRouter.hasCapacity(fallback)) {
                actualModel = fallback;
                // Try with a fresh key for the fallback model
                const { key: fbKey, keyIndex: fbKeyIdx } = ModelRouter.getNextApiKey(actualModel);
                const fbProvider = new GeminiProvider(fbKey);
                response = await fbProvider.generateCompletion(messages, toolDefs, systemPrompt, actualModel);
                ModelRouter.recordRequest(actualModel, fbKeyIdx);
                retried = true;
                break;
              }
            }
      ...
      // 4. Log Response (include routing info)
      core.addResponseLog({
        agentIndex: this.host.data.index,
        agentName: this.host.data.name,
        ...
        raw: { ...response.raw, model: actualModel, taskCategory: category },
        taskId: this.host.getCurrentTaskId() || undefined
      });
  ...
  public async spark() {
    return this.think('Start the project by proposing initial tasks.', { silent: true, taskCategory: 'orchestrate' });
  }

  /** Autonomous Intent: Work on a specific task. */
  public async executeTask(taskId: string) {
    return this.think(`Proceed with task: ${taskId}`, { silent: true, taskCategory: 'execute' });
  }

  /** Autonomous Intent: Finalize and deliver the project results. */
  public async concludeProject() {
    return this.think('All tasks are complete! Use the deliver_project tool to fulfill the final delivery with the project result.', { silent: true, taskCategory: 'deliver' });
  }
  ...
  public async processFinalAsset(prompt: string, options: any) {
    ...
    try {
      ...
      const { key: assetKey } = ModelRouter.getNextApiKey(model);
      const provider = new GeminiProvider(assetKey) as any;
